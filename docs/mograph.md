...
- After every edit: `lintFilm` (unknown colour, easing, preset, shape, effect, camera preset,
  camera track or transition type; a gradient stop or a colour keyframe that is not a colour; an in
  past the scene, a group's delay counted in; an out before the in has finished; a stagger that runs
  past the scene, or a preset that needs one and has none; a handover longer than the scene; a group
  box that does not fit a format; an empty group; text that does not hold long enough to read; a
  missing image; more particles than a frame may cost; ids, a group's children included).
