...
export type VCtx = { film: MgFilm; scene: MgScene; fr: Frame; frame: number; /** frames the groups above this layer push its in back by */ delay?: number };
...
export const TextFx: React.FC<{ ctx: VCtx; layer: TextLayer; style: React.CSSProperties; accent: string; preset: string; markStyle?: React.CSSProperties; paint?: Paint }> = ({ ctx, layer, style, accent, preset, markStyle, paint }) => {
  ...
  if (preset === "scramble") {
    const p = inProgress(film, scene, layer, frame, ctx.delay ?? 0);
  ...
  return (
    <div style={{ ...style, perspective: `${600 * ctx.fr.u}px` }}>
      {paintSpan(chars.map((c, i) => {
        const delay = (ctx.delay ?? 0) + staggerDelay(st, i, chars.length);
...
export const ShapeSvg: React.FC<{ ctx: VCtx; layer: ShapeLayer; pose: Pose; fill: string; stroke: string }> = ({ ctx, layer, pose, fill: fillCss, stroke: strokeCss }) => {
  ...
  const drawn = drawnProgress(layer.progress, !!layer.tracks?.progress, pose.progress, inProgress(film, scene, layer, frame, ctx.delay ?? 0));
...
export const LineChartView: React.FC<{ ctx: VCtx; layer: LineChartLayer; pose: Pose }> = ({ ctx, layer, pose }) => {
  ...
  const drawn = drawnProgress(undefined, !!layer.tracks?.progress, pose.progress, inProgress(film, scene, layer, frame, ctx.delay ?? 0));
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 40 * u }}>
      <svg width={dia} height={dia} viewBox={`0 0 ${dia} ${dia}`} style={{ display: "block", transform: "rotate(-90deg)" }}>
        ...
        {layer.values.map((v, i) => {
          ...
          const drawn = drawnProgress(undefined, !!layer.tracks?.progress, pose.progress, inProgress(film, scene, layer, frame, (ctx.delay ?? 0) + delay));
      ...
      {layer.legend === false ? null : (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 * u, fontFamily: fontFor(film), fontWeight: 500, fontSize: labelSize, color: labelColor }}>
          {layer.values.map((v, i) => {
            const p = poseAt(film, scene, layer, frame, (ctx.delay ?? 0) + staggerDelay(st, i, layer.values.length));
            return (
...
export const ParticlesView: React.FC<{ ctx: VCtx; layer: ParticlesLayer }> = ({ ctx, layer }) => {
  ...
  const parts = particlesAt({ ...layer, count: Math.min(layer.count ?? 60, MAX_PARTICLES), w: boxW, h: boxH }, Math.max(0, frame - t.inAt - (ctx.delay ?? 0)), { w: boxW, h: boxH });
  return (
