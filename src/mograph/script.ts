...
Sound: a layer may carry "sound": "pop" | "tick" | "click" | "whoosh" | "rise" | "thud" | "ding" | "swell" (a synthesised bank the harness writes) or { "name", "gain", "at": frames after the in }; a scene may carry "sound" at its start. Use one hit per scene at most, on the layer that lands (a counter, a pop, a slide), and a "swell" under a title.
Groups, camera, handovers: a "group" layer ({ "type": "group", "w", "h", "fill"?, "radius"?, "layers": [...] } with children's "at" as fractions of the box, and a stagger by item) for a card or a panel; "camera": { "preset": "push" | "pull" | "pan" | "tilt" | "drift" | "orbit", "from", "to", "focus" } on a scene for a slow move, or "tracks": { "x": [{ "at", "v" }, ...] } for a travelling shot; "transition": { "type": "dissolve" | "dip" | "push-left" | "push-up" | "wipe-left" | "zoom" | "blur", "dur": 10 to 16 } on the incoming scene instead of a plain cut when the film should flow.
Colour and effects: any colour may be { "gradient": ["ink", "#0F3D5E"], "angle": 160 }; "colorTracks": { "fill": [{ "at", "v" }] } animates a colour; "effects": { "shadow": {}, "glow": { "color" }, "highlight": { "color" } (a marker sweep on *marks*), "gradientText": ["accent", "rose"] }; text presets also include "flip" (with a char stagger), "track", "scramble", "fall", "line-wipe"; shapes also "path" (d, viewBox, drawn by a progress track), "polygon", "star", "arrow"; layers also "line" (a line chart), "rings", "particles" (count up to 400, seeded). Use one or two of these per film where they carry meaning, never all at once.
