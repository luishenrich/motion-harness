...
import type { BarsLayer, CounterLayer, GroupLayer, ImageLayer, Layer, LineChartLayer, ListLayer, MgFilm, MgScene, MgTransition, ParticlesLayer, RingsLayer, ShapeLayer, TextLayer, TransitionType } from "./schema.ts";
...
/** the layer's own lint flags, plus "none" while this copy of the scene only sits under a transition */
const boxFlags = (ctx: Ctx, layer: Layer): string | undefined => {
  const own = lintFlags(layer);
  if (!ctx.under) return own;
  const flags = new Set([...(own ?? "").split(" ").filter(Boolean), "none"]);
  return [...flags].join(" ");
};
...
const Box: React.FC<{ ctx: Ctx; layer: Layer; pose: Pose; children: React.ReactNode; extra?: React.CSSProperties; lines?: number; origin?: string }> = ({ ctx, layer, pose, children, extra, lines, origin }) => {
  ...
  return (
    <div data-probe={off ? undefined : layer.id} data-mg={ctx.under ? undefined : `${ctx.scene.id}.${ctx.path}${layer.id}`} data-lines={lines} data-lint={boxFlags(ctx, layer)} style={{ ...base, visibility: pose.visible ? "visible" : "hidden" }}>
      <div style={{ ...poseStyle({ ...pose, rotate: 0 }, ctx.fr), transformOrigin: origin ?? "50% 50%" }}>
        <Fx ctx={ctx} layer={layer}>
          {pose.rotate ? <div style={{ transform: `rotate(${pose.rotate}deg)`, transformOrigin: origin ?? "50% 50%" }}>{children}</div> : children}
        </Fx>
      </div>
    </div>
  );
...
const TextView: React.FC<{ ctx: Ctx; layer: TextLayer }> = ({ ctx, layer }) => {
  ...
  const hl = highlightAt(film, ctx.scene, layer, frame, fr.u, ctx.delay);
    ...
    <Box ctx={ctx} layer={layer} pose={{ ...whole, opacity: whole.visible ? 1 : 0, x: 0, y: 0, scale: 1, blur: 0, wipe: 1 }} lines={expectLines}>
      <div style={style}>
        {units.map((u, i) => {
          ...
          const inner = <span data-lint={quiet(ctx)} style={{ display: lines && preset !== "line-wipe" ? "block" : "inline-block", ...poseStyle(p, fr), whiteSpace: lines ? "pre-wrap" : "pre" }}>{body}</span>;
...
const CounterView: React.FC<{ ctx: Ctx; layer: CounterLayer }> = ({ ctx, layer }) => {
  ...
  return (
    <Box ctx={ctx} layer={layer} pose={p}>
      <div data-lint={quiet(ctx)} style={{ ...box, ...textStyle(paint) }}>
...
const ChartLineView: React.FC<{ ctx: Ctx; layer: LineChartLayer }> = ({ ctx, layer }) => {
  const p = poseAt(ctx.film, ctx.scene, layer, ctx.frame, ctx.delay);
...
const GroupView: React.FC<{ ctx: Ctx; layer: GroupLayer }> = ({ ctx, layer }) => {
  ...
  const paint: React.CSSProperties = {
    background: layer.fill ? paintOf(film.design, layer.fill, film.design.paper) : undefined,
    borderRadius: layer.radius ? layer.radius * fr.u : undefined,
    border: layer.stroke ? `${(layer.thickness ?? 2) * fr.u}px solid ${flatOf(film.design, layer.stroke, film.design.ink)}` : undefined,
...
export const MgSceneView: React.FC<{ film: MgFilm; scene: MgScene; format: string; at?: number; under?: boolean; enterFrames?: number }> = ({ film, scene, format, at, under, enterFrames }) => {
  ...
  const flat = groundFlat(film, scene, frame);
  ...
  const painted = ground.gradient || ground.animated ? <AbsoluteFill style={backgroundStyle(ground)} data-lint={under ? "none color-track" : ground.animated ? "color-track" : undefined} /> : null;
  const layers = scene.layers.map((l) => <LayerView key={l.id} ctx={ctx} layer={layerFor(l, format)} />);
  return (
    <AbsoluteFill style={{ backgroundColor: flat, overflow: "hidden" }} data-mg-scene={under ? undefined : scene.id} data-mg-under={under ? scene.id : undefined} data-probe={under ? `under:${scene.id}` : undefined} data-lint={under ? "none" : undefined}>
      {scene.camera?.ground ? (
        <AbsoluteFill style={cam}>
          <AbsoluteFill style={{ backgroundColor: flat }} />
          {painted}
