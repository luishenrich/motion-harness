...
import { shotAtTime, type Asset } from "../ingest/ingest.ts";
...
export const normalizeScript = (s: Script, assets?: Asset[]): Script => {
  ...
  const scenes = s.scenes.map((sc, i) => {
    ...
    if (a && kind !== "text") {
      const shot = shotAtTime(a, Number(sc.in) || 0);
      if (shot) ground = shot.luma < 110 ? "dark" : "light";
    }
...
export const scaffoldFiles = (script: Script, opts: { harnessImport: string; formats: string[]; fps?: number; assets?: Asset[] }): Record<string, string> => {
  ...
  const fillOf = (sc: ScriptScene) => {
    const a = sc.asset ? assets.find((k) => k.id === sc.asset) : undefined;
    return a && sc.kind !== "text" ? shotAtTime(a, sc.in ?? 0)?.edge ?? "" : "";
  };
  const sceneLines = script.scenes.map((sc) => `  ${ts(sc.id)}: { dur: ${Math.round(sc.seconds * fps)}, kind: ${ts(sc.kind)}, ground: ${ts(sc.ground)}, fill: ${ts(fillOf(sc))}, headline: ${ts(sc.headline)}, body: ${ts(sc.body ?? "")}, asset: ${ts(sc.asset ?? "")}, inAt: ${sc.in ?? 0}, focus: ${sc.focus ? `[${sc.focus.join(", ")}]` : "null"}, fit: ${ts(sc.fit ?? "auto")}, zoom: ${sc.zoom ?? 0}, visual: ${ts(sc.visual ?? "")}, why: ${ts(sc.why ?? "")} },`).join("\n");
  const timeline = `/**
 * ${script.title}: the timeline as data. Generated by mh new from the brief; edit here, never in the components.
 */
import { defineTimeline } from ${ts((opts.harnessImport.startsWith(".") ? "../" : "") + opts.harnessImport + "/timeline/schema.ts")};
...
/** ground: ink or paper; fill: the exact colour footage sits on (its own edge colour, measured by ingest), empty for the design's ground; focus: the script's own point of interest (null lets the measured subject frame the crop); fit auto: cover, contain for an interface in a frame of the other orientation */
export type SceneSpec = { dur: number; kind: "text" | "clip" | "image"; ground: "dark" | "light"; fill: string; headline: string; body: string; asset: string; inAt: number; focus: [number, number] | null; fit: "cover" | "contain" | "auto"; zoom: number; visual: string; why: string };
...
import { compile } from ${ts((opts.harnessImport.startsWith(".") ? "../" : "") + opts.harnessImport + "/timeline/schema.ts")};
...
/** the ground a scene sits on: footage on its own edge colour, so a letterbox has no seam; text on the film's ink or paper */
const groundOf = (s: SceneSpec) => s.fill || (s.ground === "light" ? DESIGN.paper : DESIGN.ink);
...
import { timeline, DESIGN, SCENES } from "./src/timeline.ts";
...
export default defineConfig({
  ...
  // the footage's own edge colours (SCENES[].fill, measured by ingest) are grounds by design, not stray paint
  tokens: { colors: [DESIGN.ink, DESIGN.paper, DESIGN.accent, DESIGN.muted, "#FFFFFF", "#000000", ...Object.values(SCENES).map((s) => s.fill).filter(Boolean)], sources: ["src/**/*.tsx"] },
