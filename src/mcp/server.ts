...
// motion graphics as data (film.mograph.json): read, edit by address, lint after every edit
server.registerTool("mh_layers", { title: "Layers of a motion graphics film", description: "Every scene and layer of film.mograph.json with its address (scene.layer), timing (in preset, at, dur, stagger; out) and content. json=true returns the whole film (or one scene).", inputSchema: { scene: z.string().optional(), json: z.boolean().optional(), project } }, async ({ scene, json, project: p }) => text(await runMh("layers", scene ? [scene] : [], p, !!json)));
server.registerTool("mh_get", { title: "Read a value by address", description: "One value of the film as JSON: hook.line, hook.line.in, hook.dur, design, audio.bed.", inputSchema: { address: z.string(), project } }, async ({ address, project: p }) => text(await runMh("get", [address], p)));
server.registerTool("mh_set", { title: "Set a value by address", description: "Change one value of film.mograph.json: hook.line.size 110, hook.line.at {x,y}, hook.line.in {preset,at,dur,ease,stagger}, hook.dur 96, design.accent '#FF6B35'. Objects and arrays as JSON. The film is linted and the frame to look at is named.", inputSchema: { address: z.string(), value: z.union([z.string(), z.number(), z.boolean(), z.record(z.string(), z.unknown()), z.array(z.unknown())]), project } }, async ({ address, value, project: p }) => text(await runMh("set", [address, typeof value === "string" ? value : JSON.stringify(value)], p)));
server.registerTool("mh_unset", { title: "Unset a value", description: "Remove a property (back to its default); whole scenes and layers go through mh_remove.", inputSchema: { address: z.string(), project } }, async ({ address, project: p }) => text(await runMh("unset", [address], p)));
server.registerTool("mh_key", { title: "Keyframe", description: "Add or replace a keyframe on a layer track (opacity, x, y, scale, rotate, blur, progress, wipe, w, h); explicit tracks win over the presets. remove=true deletes the key at that frame.", inputSchema: { address: z.string().describe("scene.layer.prop"), frame: z.number(), value: z.number().optional(), ease: z.string().optional(), remove: z.boolean().optional(), project } }, async ({ address, frame, value, ease, remove, project: p }) => text(remove ? await runMh("unkey", [address, String(frame)], p) : await runMh("key", [address, String(frame), String(value ?? 0), ...(ease ? ["--ease", ease] : [])], p)));
server.registerTool("mh_add", { title: "Add a scene or a layer", description: "Add a scene ({id,dur,ground,exit,layers}) or a layer to a scene ({id,type,...}); after/before place it. See docs/mograph.md for the layer fields.", inputSchema: { scene: z.string().optional().describe("the scene to add a layer to; omit to add a scene"), item: z.record(z.string(), z.unknown()), after: z.string().optional(), before: z.string().optional(), project } }, async ({ scene, item, after, before, project: p }) => text(await runMh("add", scene ? ["layer", scene, JSON.stringify(item), ...(after ? ["--after", after] : []), ...(before ? ["--before", before] : [])] : ["scene", JSON.stringify(item), ...(after ? ["--after", after] : []), ...(before ? ["--before", before] : [])], p)));
server.registerTool("mh_remove", { title: "Remove a scene or a layer", description: "Remove a whole scene (hook) or a layer (hook.rule).", inputSchema: { address: z.string(), project } }, async ({ address, project: p }) => text(await runMh("remove", [address], p)));
server.registerTool("mh_move", { title: "Move a scene or a layer", description: "Reorder: a scene among scenes, a layer among its scene's layers (drawing order).", inputSchema: { address: z.string(), after: z.string().optional(), before: z.string().optional(), project } }, async ({ address, after, before, project: p }) => text(await runMh("move", [address, ...(after ? ["--after", after] : []), ...(before ? ["--before", before] : [])], p)));
server.registerTool("mh_layout", { title: "Layout", description: "Stacked blocks pushed apart and kept in the safe band, per format; run when the rendered lint reports a collision.", inputSchema: { scene: z.string().optional(), project } }, async ({ scene, project: p }) => text(await runMh("layout", scene ? [scene] : [], p)));
server.registerTool("mh_new_mograph", { title: "New motion graphics film", description: "A model writes film.mograph.json from a brief, the project is scaffolded (native engine, no Remotion), mh check runs on it. Then edit by address with mh_set and look with mh_frame.", inputSchema: { dir: z.string(), brief: z.string(), seconds: z.number().optional(), formats: z.array(z.string()).optional(), language: z.string().optional(), force: z.boolean().optional() } }, async ({ dir, brief, seconds, formats, language, force }) => text(await runMh("new", [dir, "--mograph", "--brief", brief, ...(seconds ? ["--seconds", String(seconds)] : []), ...(formats?.length ? ["--formats", formats.join(",")] : []), ...(language ? ["--language", language] : []), ...(force ? ["--force"] : [])])));
